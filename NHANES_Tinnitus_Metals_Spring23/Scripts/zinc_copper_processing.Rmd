...
# refactor, compute, combine for various variables
  mutate(
         ...
         "age_group" = if_else(age_yrs < 30, 1, # youngest individuals with audio data are 20
                               if_else(age_yrs < 40, 2, # so age ranges are 20-29, 30-39, 40-49, 50-59, 60-69
...
# remove remaining unwanted variables
  select(c(SEQN, cycle, sex, age_yrs, age_group, ethnicity, combined_subsample_weight, psu, stratum, tinnitus_any, tinnitus_type, zinc, copper, speech_in_noise, zinc_deficiency, copper_deficiency))
...
In both cases we can see that these methods have reduced the skewness of the distribution, so we will attempt analyses using the transformed zinc and copper distributions. Appending the vectors of transformed values to our final data frame.
